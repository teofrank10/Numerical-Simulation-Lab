...
int main (){
	...
	const int M_equilib = 1510; //euilibration steps
	...
	const double delta_100 = 1.2245; //set the parmeters for the uniform sampling
	const double delta_210 = 2.9699;
	...
	ofstream outfile("data/Unif/info_ex5.1.dat");
	if (!outfile.is_open()) {
	...
	outfile_points.open("data/Unif/eq_positions100.dat");
	if (!outfile_points.is_open()){
		cout << "Error: unable to open eq_positions100.dat" << endl;
		exit(1);
	}
	
	outfile_radii.open("data/Unif/eq_radii100.dat");
...
//===================Uniform sampling=====================//
	...
	for (int i = 0; i < M_equilib; i++){//use metropolis to equilibrate the system
		Rand.Metropolis_Unif(pos_100, delta_100, psi_100, 3);
	...
	outfile_points.open("data/Unif/eq_positions210.dat");
	if (!outfile_points.is_open()){
		cout << "Error: unable to open eq_positions210.dat" << endl;
		exit(1);
	}
	
	outfile_radii.open("data/Unif/eq_radii210.dat");
	if (!outfile_radii.is_open()){
	...
	for (int i = 0; i < M_equilib; i++){
		Rand.Metropolis_Unif(pos_210, delta_210, psi_210, 3);
	...
	outfile_100.open("data/Unif/positions_100.dat");
	if (!outfile_100.is_open()){
		cout << "Error: unable to open positions_100.dat" << endl;
		exit(1);
	}
	
	outfile_210.open("data/Unif/positions_210.dat");
	...
	outfile_100.open("data/Unif/r_100.dat");
	if (!outfile_100.is_open()){
		cout << "Error: unable to open positions_100.dat" << endl;
		exit(1);
	}
	
	outfile_210.open("data/Unif/r_210.dat");
	if (!outfile_210.is_open()){
		cout << "Error: unable to open positions_210.dat" << endl;
		exit(1);
	}

...
double variationalWaveFunction(double v)
{
	...
	double mu = 0.8;
	double sigma = 0.62;
	double exp1 = -pow((v - mu),2);
	double exp2 = -pow((v + mu),2);
...
double variationalWaveFunction_second(double v)
{
	...
	double exp2 = -pow((v + mu),2);
	double denom = 2*pow(sigma,2);
	return -variationalWaveFunction(v)/pow(sigma,2) - exp1/pow(sigma,4)*exp(exp1/denom) - exp2/pow(sigma,4)*exp(exp2/denom);
